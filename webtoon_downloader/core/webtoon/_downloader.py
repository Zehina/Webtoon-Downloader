...
#     Args:
#         session: the requests session object for persistent parameters.
#         viewer_url: chapter reader url of the series.
#         start_chapter: starting range of chapters to download.
#         end_chapter: end range of chapter to download, inclusive of this chapter number. Defaults to the last chapter detected

#     Returns:
#         A list of all chapter details extracted.
#     """
...
#         session: the requests session object for persistent parameters.
#         viewer_url: chapter reader url of the series.
#         data_episode_num: chapter number to scrap image urls from.
...
#     Args:
#         chapter_download_task_id: task of calling chapter download task
#         session: the requests session object for persistent parameters.
#         viewer_url: chapter reader url of the series.
#         chapter_number: chapter to download
#         zeros: Number of padding digits used for naming the saved image file.
#         dest: destination folder path to store the downloaded image files. Defaults to the current working directory
#         exporter: An optional TextExporter
#     """
#     log.debug(
...
#     extractor = WebtoonViewerPageExtractor(html)  # TODO add exporter to downloader2
#     img_urls = extractor.get_img_urls()
#     if not os.path.exists(dest):  # TODO add to downloader2 ?
#         os.makedirs(dest)
#     if exporter:
...
#     Args:
#         series_url: url of the series to scrap from the webtoons.com website, The URL provided should be in the following format https://www.webtoons.com/en/{?}/{?}/list?title_no={?}
#         start_chapter: starting range of chapters to download. Defaults to the first chapter detected
#         end_chapter: End range of chapter to download, inclusive of this chapter number. Defaults to the last chapter detected
#         dest: destination folder path to store the downloaded image files of the chapter. Defaults to the current working directory
#         separate_chapters: separate downloaded chapters in their own folder under the dest path if true, else stores all images in the dest folder.
#         exporter: An optional TextExporter
#     """
...
#             chapter_dest = os.path.join(
#                 dest, get_chapter_dir(chapter_info, zeros, separate_chapters)
#             )
...
#                     _chapters_iterable, N_CONCURRENT_CHAPTERS_DOWNLOAD
#                 )
#             }
#             while chapter_download_futures:
#                 done, chapter_download_futures = wait(
#                     chapter_download_futures, return_when=FIRST_COMPLETED
#                 )
#                 for fut in done:
#                     if fut.exception():
#                         log.error(
...
#                         prepare_chapter_download(next(_chapters_iterable))
#                     )
#     return n_downloaded_chapters
...
#         dest: folder path where to save the downloaded image.
#         chapter_number: chapter number used for naming the saved image file.
#         page_number: page number used for naming the saved image file.
#         zeros: Number of digits used for the chapter number
#         image_format: format of downloaded image. (default: jpg)
#         page_digits: Number of digits used for the page number inside the chapter
#     """
